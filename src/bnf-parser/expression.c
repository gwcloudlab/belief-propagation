...
static void insert_edges_into_graph(char * variable_buffer, int num_node_names, float * probability_buffer, int num_probabilities, Graph_t graph){
	...
	slice = (int)(num_probabilities / graph->node_states_size[dest_index]);
...
*/
	...
	for(i = num_node_names - 1; i > 0; --i){
        ...
        delta = (int)graph->node_states_size[src_index];
		...
		graph_add_edge(graph, (size_t )src_index, (size_t )dest_index, graph->node_states_size[src_index], graph->node_states_size[dest_index]);
		if(graph->observed_nodes[src_index] != 1 ){
			graph_add_edge(graph, (size_t)dest_index, (size_t )src_index, graph->node_states_size[dest_index], graph->node_states_size[src_index]);
		}
