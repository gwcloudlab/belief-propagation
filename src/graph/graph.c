...
void init_edge(Graph_t graph, unsigned int edge_index, unsigned int src_index, unsigned int dest_index, unsigned int dim_x,
    ...
    for(i = 0; i < dim_x; ++i){
        ...
        graph->edges_messages[edge_index].previous_sum = INFINITY;
        graph->edges_messages[edge_index].current_sum = INFINITY;
    }
}
...
void send_message(struct belief *states, unsigned int edge_index, struct joint_probability *edge_joint_probabilities, struct belief *edge_messages){
    ...
    edge_messages[edge_index].previous_sum = edge_messages[edge_index].current_sum;
    edge_messages[edge_index].current_sum = sum;
	for (i = 0; i < num_src; ++i) {
...
#pragma acc routine
static inline void combine_page_rank_message(struct belief * dest, struct belief *src, unsigned num_variables, unsigned int offset){
    int i;
...
#pragma omp parallel for default(none) shared(current_edge_messages, num_edges)  private(j, diff) reduction(+:delta)
        for(j = 0; j < num_edges; ++j){
            diff = current_edge_messages[j].previous_sum - current_edge_messages[j].current_sum;
...
#pragma acc kernels
            for (j = 0; j < num_edges; ++j) {
                diff = curr_messages[j].previous_sum - curr_messages[j].current_sum;
                if (diff != diff) {
                    diff = 0.0f;
                }
                delta += fabs(diff);
