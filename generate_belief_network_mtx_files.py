...
def parse_arguments():
    ...
    parser.add_argument("--num-beliefs", type=int, default=2, help="The number of beliefs")
    ...
    assert args.num_beliefs > 0, "The number of beliefs (%d) must be greater than 0." % args.num_beliefs
    return args
...
def write_observed_nodes(mtx, args):
    ...
    for i in population:
        mtx_id = i + 1
        if i in samples:
            mtx.write("{:d}\t{:d}\t".format(mtx_id, mtx_id))
            sum = 0.0
            probs = []
            beliefs = []
            for j in range(args.num_beliefs):
                prob = random.random()
                sum += prob
                probs.append(prob)
            for i in range(len(probs)):
                probs[i] = probs[i] / sum
                beliefs.append("{:f}".format(probs[i]))
...
def write_and_build_tree(mtx, args):
    parents = {}
...
def write_edges(mtx, args):
    num_arcs_written = 0
    parents = write_and_build_tree(mtx, args)
...
def generate_file(args):
    ...
    with open(temp_file, 'w') as mtx_edges:
        with open(mtx_nodes_file, 'w') as mtx_nodes:
            ...
            write_observed_nodes(mtx_nodes, args)
            num_arcs_written = write_edges(mtx_edges, args)
